...
def main():
  ...
  model.load_state_dict(torch.load("result/20221124000716-model_FDSR-epochs_1000/trained_model.pt"))
  ...
  print("Pixel error: " + str(pixel_error(tensor_output, hr_depth_map, t="fake").item()))

  n = Normalization('lr-4-warior')

  lr_pcl = PointCloud(n.recover_normalized_sample(lr_depth_map, "depth")[0][0].numpy())
  hr_pcl = PointCloud(n.recover_normalized_sample(hr_depth_map, "depth")[0][0].numpy())
  out_pcl = PointCloud(n.recover_normalized_sample(tensor_output, "depth")[0][0].numpy())
  ...
  plt.imshow(n.recover_normalized_sample(hr_depth_map, "depth")[0][0], cmap=cmap, vmin=500.0)

  plt.figure(plt.figure('Predicted HR Depth map'))
  plt.imshow(n.recover_normalized_sample(tensor_output, "depth")[0][0], cmap=cmap, vmin=500.0)
